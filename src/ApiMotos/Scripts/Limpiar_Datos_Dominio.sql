...
-- Etapa B (pedidos) y Etapa C (historial de precios). Van ANTES de Envíos y Variantes:
-- PC_ACTIVIDADES.PedidoId -> PC_PEDIDOS -> PC_ENVIOS, y PC_PRECIO_HISTORIAL -> PC_VARIANTES.
IF OBJECT_ID('PC_PRECIO_HISTORIAL', 'U') IS NOT NULL DELETE FROM PC_PRECIO_HISTORIAL;
IF OBJECT_ID('PC_PEDIDO_LINEAS', 'U') IS NOT NULL DELETE FROM PC_PEDIDO_LINEAS;
IF OBJECT_ID('PC_PEDIDOS', 'U') IS NOT NULL DELETE FROM PC_PEDIDOS;
...
DECLARE cur CURSOR LOCAL FAST_FORWARD FOR
    ...
    WHERE name IN ('PC_ACTIVIDADES','PC_METAS','PC_PRECIO_HISTORIAL','PC_PEDIDO_LINEAS','PC_PEDIDOS','PC_OBSERVACIONES','PC_ENVIOS','PC_MOVIMIENTOS_STOCK',
