...
using ApiMotos.Application.Agregates.Envios.Commands.Eliminar;
using ApiMotos.Application.Agregates.Envios.Queries.Envios;
using ApiMotos.Application.Agregates.Envios.Queries.Resumen;
using ApiMotos.Application.Agregates.Envios.Queries.Buscar;
using ApiMotos.Application.Agregates.Envios.Queries.ByClienteId;
using ApiMotos.Application.Agregates.Envios.Queries.ByAgenciaId;
using ApiMotos.Application.Agregates.Envios.Commands.Transicion;
...
namespace ApiMotos.Controllers
{
    ...
    public class EnvioController : ControllerBase
    {
        ...
        {
            var query = new EnviosQuery(Skip: skip, Take: take);
        ...
        public async Task<IActionResult> GetById([FromRoute] int id)
        {
            ...
            var query = new EnviosQuery(Id: id);
            var result = await _mediator.Send(query);
            var item = result.FirstOrDefault();
            if (item == null)
                return NotFound($"Envio con ID {id} no encontrado");
            return Ok(item);
        }
        ...
        {
            var result = await _mediator.Send(new EnviosResumenQuery());
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new EnviosBuscarQuery(q ?? ""));
            return Ok(result);
        }
        [HttpGet("by-cliente/{id}")]
        public async Task<IActionResult> GetByClienteId([FromRoute] int id)
        {
            var query = new EnviosByClienteIdQuery { ClienteId = id };
            var result = await _mediator.Send(query);
            return Ok(result);
        }

        [HttpGet("by-agencia/{id}")]
        public async Task<IActionResult> GetByAgenciaId([FromRoute] int id)
        {
            var query = new EnviosByAgenciaIdQuery { AgenciaId = id };
            var result = await _mediator.Send(query);
            return Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CrearEnvioCommand command)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] ModificarEnvioCommand command)
        {
            if (id != command.Id)
        ...
        {
            var command = new EliminarEnvioCommand(id);
            var result = await _mediator.Send(command);
            if (result.IsSuccess)
                return Ok(result.Value);
            // RFC 7807 problem+json (Ola 1): errors[].message conserva el contrato del form.
        ...
        /// <summary>
        /// Transiciones del ciclo de vida (pasar-a-facturado, pasar-a-despachado, pasar-a-entregado, pasar-a-anulado) — la matriz vive en ciclos-vida.json.
        /// </summary>
        [HttpPost("{id}/{accion}")]
        public async Task<IActionResult> Transicion([FromRoute] int id, [FromRoute] string accion)
        {
            var result = await _mediator.Send(new TransicionEnvioCommand(id, accion));
            if (result.IsSuccess)
                return Ok(result.Value);
            // RFC 7807 problem+json (Ola 1): errors[].message conserva el contrato del form.
            return Rechazo(result.Errors.Select(e => e.Message));
        ...
        {
            var query = new EnviosQuery();
            var result = await _mediator.Send(query);
            var entity = result.FirstOrDefault(x => x.Id == id);

            if (entity == null)
                return NotFound(new { canExecute = false, errors = new[] { new { code = "NOT_FOUND", message = "Envio no encontrado" } } });
