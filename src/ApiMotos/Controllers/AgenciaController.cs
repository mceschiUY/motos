...
using ApiMotos.Application.Agregates.Agencias.Commands.Eliminar;
using ApiMotos.Application.Agregates.Agencias.Queries.Agencias;
using ApiMotos.Application.Agregates.Agencias.Queries.Resumen;
using ApiMotos.Application.Agregates.Agencias.Queries.Buscar;
...
namespace ApiMotos.Controllers
{
    ...
    public class AgenciaController : ControllerBase
    {
        ...
        public AgenciaController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            var query = new AgenciasQuery(Skip: skip, Take: take);
        ...
        public async Task<IActionResult> GetById([FromRoute] int id)
        {
            ...
            var query = new AgenciasQuery(Id: id);
            var result = await _mediator.Send(query);
            var item = result.FirstOrDefault();
            if (item == null)
                return NotFound($"Agencia con ID {id} no encontrado");
            return Ok(item);
        }
        ...
        {
            var result = await _mediator.Send(new AgenciasResumenQuery());
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new AgenciasBuscarQuery(q ?? ""));
            return Ok(result);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CrearAgenciaCommand command)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] ModificarAgenciaCommand command)
        {
            if (id != command.Id)
        ...
        {
            var command = new EliminarAgenciaCommand(id);
            var result = await _mediator.Send(command);
            if (result.IsSuccess)
                return Ok(result.Value);
            // RFC 7807 problem+json (Ola 1): errors[].message conserva el contrato del form.
        ...
        {
            var query = new AgenciasQuery();
            var result = await _mediator.Send(query);
            var entity = result.FirstOrDefault(x => x.Id == id);

            if (entity == null)
                return NotFound(new { canExecute = false, errors = new[] { new { code = "NOT_FOUND", message = "Agencia no encontrado" } } });
