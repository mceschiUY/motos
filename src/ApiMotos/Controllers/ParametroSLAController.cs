...
using ApiMotos.Application.Agregates.ParametroSLAs.Commands.Eliminar;
using ApiMotos.Application.Agregates.ParametroSLAs.Queries.ParametroSLAs;
using ApiMotos.Application.Agregates.ParametroSLAs.Queries.Resumen;
using ApiMotos.Application.Agregates.ParametroSLAs.Queries.Buscar;
...
namespace ApiMotos.Controllers
{
    ...
    public class ParametroSLAController : ControllerBase
    {
        private readonly IMediator _mediator;

        public ParametroSLAController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            var query = new ParametroSLAsQuery(Skip: skip, Take: take);
        ...
        public async Task<IActionResult> GetById([FromRoute] int id)
        {
            ...
            var query = new ParametroSLAsQuery(Id: id);
            var result = await _mediator.Send(query);
            var item = result.FirstOrDefault();
            if (item == null)
                return NotFound($"ParametroSLA con ID {id} no encontrado");
            return Ok(item);
        }
        ...
        {
            var result = await _mediator.Send(new ParametroSLAsResumenQuery());
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new ParametroSLAsBuscarQuery(q ?? ""));
            return Ok(result);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CrearParametroSLACommand command)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] ModificarParametroSLACommand command)
        {
            if (id != command.Id)
        ...
        {
            var command = new EliminarParametroSLACommand(id);
            var result = await _mediator.Send(command);
            if (result.IsSuccess)
                return Ok(result.Value);
            // RFC 7807 problem+json (Ola 1): errors[].message conserva el contrato del form.
        ...
        {
            var query = new ParametroSLAsQuery();
            var result = await _mediator.Send(query);
            var entity = result.FirstOrDefault(x => x.Id == id);

            if (entity == null)
                return NotFound(new { canExecute = false, errors = new[] { new { code = "NOT_FOUND", message = "ParametroSLA no encontrado" } } });
