...
export class ActividadListComponent implements OnInit, OnDestroy {
  ...
  claseDeEstado(item: any): string {
    const valores = ACTIVIDAD_DESCRIPTOR.campos.find(c => c.nombre === 'resultado')?.valores ?? [];
    const i = valores.indexOf((item.resultado ?? '').toString());
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('actividad');
  ...
  readonly descriptor = ACTIVIDAD_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'resultado', label: 'Resultado', visible: true, sortable: true },
    { key: 'proximaAccion', label: 'Próxima acción', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Actividad) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params: any) => {
      if (params['clienteId']) {
        ...
        this.breadcrumbItems = [
          ...
          { label: `Actividad de Cliente #${this.filterByClienteId}`, icon: 'list_alt' }
        ];
      }
      if (params['vendedorId']) {
        ...
        this.breadcrumbItems = [
          ...
          { label: `Actividad de Vendedor #${this.filterByVendedorId}`, icon: 'event_note' }
        ];
      }
      this.loadData();
    });
    ...
    this.puenteVoz.registrarAbridor('actividad', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('actividad');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('actividad');
    const validas: string[] = ['table', 'cards', 'master-detail', 'timeline', 'calendario', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('actividad-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('actividad-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[ActividadList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[ActividadList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  selectItem(item: Actividad): void {
  ...
  async deleteItem(item: Actividad): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[ActividadList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Actividad) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[ActividadList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'actividad_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'actividad_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Actividad[], filename: string): void {
  ...
  openDocuments(item: Actividad): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Actividad'
      }
    });
  }

  verFicha(item: Actividad): void {
    this.router.navigate(['/actividad', item.id]);
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: Actividad): string {
    return (item as any).clienteDisplay || `#${item.id}`;
  }

  getItemSubtitle(item: Actividad): string {
    return (item as any).descripcion || (item as any).email || '';
  }

  getStatusClass(item: Actividad): string {
    return this.claseDeEstado(item);
  }

  getStatusLabel(item: Actividad): string {
    // El "estado" de una actividad es su resultado; la etiqueta sale del descriptor.
    const valor = ((item as any).resultado ?? '').toString();
    return ACTIVIDAD_DESCRIPTOR.campos.find(c => c.nombre === 'resultado')?.etiquetas?.[valor] ?? valor;
  }
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[ActividadList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Actividad): void {
    this.router.navigate([relation.route], {
