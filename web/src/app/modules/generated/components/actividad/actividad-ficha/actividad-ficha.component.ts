...
@Component({
  ...
  styleUrl: './actividad-ficha.component.scss'
})
export class ActividadFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Actividad | null>(null);
  ...
  // Actividad no tiene hijas: sin pestañas de relacionados.
  ...
  etiquetaResultado(valor: string | null | undefined): string {
    const v = (valor ?? '').toString();
    return this.etiquetasResultado[v] ?? v.replace(/_/g, ' ');
  }
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('actividad', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('actividad');
  }
  ...
  titulo(): string {
    ...
    if (!i) { return ''; }
    const tipo = (i.tipo || 'Actividad').toString();
    const cliente = i.clienteDisplay ? ' · ' + i.clienteDisplay : '';
    return tipo.charAt(0).toUpperCase() + tipo.slice(1) + cliente;
  }

  volver(): void { this.router.navigate(['/actividad']); }
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Actividad' } });
  }
  ...
  irACliente(): void {
    const i = this.item();
    if (i?.clienteId) { this.router.navigate(['/cliente', i.clienteId]); }
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Actividad/${this.id}`).subscribe({
