...
@Component({
  ...
  templateUrl: './talla-ficha.component.html',
  styleUrl: './talla-ficha.component.scss'
})
export class TallaFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Talla | null>(null);
  ...
  readonly relaciones: RelacionFicha[] = [
    { nombre: 'variante', label: 'Variantes', icon: 'qr_code_2', ruta: '/variante', endpoint: '/Variante/by-talla/' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('talla', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('talla');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.nombre || ('Talla #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/talla']); }

  editar(): void {
    const ref = this.dialog.open(TallaFormComponent, {
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Talla' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Talla/${this.id}`).subscribe({
