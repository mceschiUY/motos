...
import { TALLA_DESCRIPTOR } from '../../../models/talla.descriptor';
...
@Component({
  ...
  styleUrl: './talla-list.component.scss'
})
export class TallaListComponent implements OnInit, OnDestroy {
  private readonly service = inject(TallaService);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('talla');
  ...
  readonly descriptor = TALLA_DESCRIPTOR;
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Talla) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  readonly gruposPorFecha = computed<TimelineGroup[]>(() => {
    ...
    const grupos = new Map<string, Talla[]>();

    items.forEach((item: Talla) => {
  ...
  selection = new SelectionModel<Talla>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Talla', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('talla', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('talla');
  }
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('talla-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[TallaList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[TallaList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Talla): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[TallaList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Talla) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[TallaList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'talla_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'talla_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Talla[], filename: string): void {
  ...
  openDocuments(item: Talla): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Talla'
      }
    });
  }

  verFicha(item: Talla): void {
    this.router.navigate(['/talla', item.id]);
  }
  ...
  getStatusLabel(item: Talla): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[TallaList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Talla): void {
    this.router.navigate([relation.route], {
