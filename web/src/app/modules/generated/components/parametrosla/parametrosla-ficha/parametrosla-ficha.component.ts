...
@Component({
  selector: 'app-parametrosla-ficha',
  standalone: true,
  ...
  templateUrl: './parametrosla-ficha.component.html',
  styleUrl: './parametrosla-ficha.component.scss'
})
export class ParametroslaFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Parametrosla | null>(null);
  ...
  estadoActual(): string { return this.normEstado(((this.item() as any) || {}).etapa); }
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('parametrosla', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('parametrosla');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.etapa || ('Parametro SLA #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/parametrosla']); }

  editar(): void {
    const ref = this.dialog.open(ParametroslaFormComponent, {
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Parametrosla' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Parametrosla/${this.id}`).subscribe({
