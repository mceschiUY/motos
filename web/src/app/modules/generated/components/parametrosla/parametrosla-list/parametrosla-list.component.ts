...
import { PARAMETROSLA_DESCRIPTOR } from '../../../models/parametrosla.descriptor';
import { ParametroslaService } from '../../../services/parametrosla.service';
import { ParametroslaFormComponent } from '../parametrosla-form/parametrosla-form.component';
...
@Component({
  selector: 'app-parametrosla-list',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './parametrosla-list.component.html',
  styleUrl: './parametrosla-list.component.scss'
})
export class ParametroslaListComponent implements OnInit, OnDestroy {
  private readonly service = inject(ParametroslaService);
  ...
  readonly kanbanListIds = ['kcol-facturacion', 'kcol-despacho', 'kcol-entrega'];
  ...
  itemsDeEstado(estado: string): Parametrosla[] {
    return this.filteredItems().filter(i => this.normEstado((i as any).etapa) === estado);
  }

  itemsSinEstado(): Parametrosla[] {
    const conocidos = ['facturacion', 'despacho', 'entrega'];
    return this.filteredItems().filter(i => !conocidos.includes(this.normEstado((i as any).etapa)));
  }

  puedeEntrarEn(destino: string): (drag: CdkDrag<Parametrosla>) => boolean {
    return (drag: CdkDrag<Parametrosla>) => {
      const desde = this.normEstado((drag.data as any)?.etapa);
      return (this.kanbanPermitidos[destino] || []).includes(desde);
    };
  }

  onKanbanDrop(event: CdkDragDrop<Parametrosla[]>, destino: string): void {
    ...
    const estadoAnterior = item.etapa;
    item.etapa = destino;               // optimista: la card se mueve ya
    this.items.set([...this.items()]);
    accion(item.id).subscribe({
      next: () => this.showMessage(this.getItemTitle(item) + ' → ' + destino.replace(/_/g, ' '), 'success'),
      error: (err: unknown) => {
        item.etapa = estadoAnterior;    // el dominio la rechazó: la card vuelve
        this.items.set([...this.items()]);
  ...
  readonly availableViews = ['table', 'cards', 'master-detail', 'kanban'];

  // ── VISTAS ARTESANALES (modules/artesanal — zona NO generada) ──
  // artesanal registrada = default de la entidad; tabla es el paracaídas.
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('parametrosla');
  ...
  readonly descriptor = PARAMETROSLA_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'etapa', label: 'Etapa', visible: true, sortable: true },
    { key: 'rangoAlertaUmbralAdvertenciaDias', label: 'Rango Alerta Umbral Advertencia Dias', visible: true, sortable: true },
    { key: 'rangoAlertaLimiteDias', label: 'Rango Alerta Limite Dias', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Parametrosla) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  readonly gruposPorFecha = computed<TimelineGroup[]>(() => {
    ...
    const grupos = new Map<string, Parametrosla[]>();

    items.forEach((item: Parametrosla) => {
  ...
  dataSource = new MatTableDataSource<Parametrosla>([]);
  selection = new SelectionModel<Parametrosla>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Parametro SLA', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('parametrosla', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('parametrosla');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('parametrosla');
    const validas: string[] = ['table', 'cards', 'master-detail', 'kanban', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('parametrosla-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('parametrosla-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[ParametroslaList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[ParametroslaList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  toggleRow(row: Parametrosla): void {
  ...
  selectItem(item: Parametrosla): void {
  ...
  async deleteItem(item: Parametrosla): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[ParametroslaList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Parametrosla) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[ParametroslaList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'parametrosla_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'parametrosla_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Parametrosla[], filename: string): void {
  ...
  openDocuments(item: Parametrosla): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Parametrosla'
      }
    });
  }

  verFicha(item: Parametrosla): void {
    this.router.navigate(['/parametrosla', item.id]);
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: Parametrosla): string {
    return (item as any).etapa || (item as any).nombre || (item as any).titulo || `#${item.id}`;
  }

  getItemSubtitle(item: Parametrosla): string {
    return (item as any).descripcion || (item as any).email || '';
  }

  getStatusClass(item: Parametrosla): string {
  ...
  getStatusLabel(item: Parametrosla): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[ParametroslaList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Parametrosla): void {
    this.router.navigate([relation.route], {
