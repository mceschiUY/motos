...
import { COLOR_DESCRIPTOR } from '../../../models/color.descriptor';
...
export class ColorListComponent implements OnInit, OnDestroy {
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'Variante', entityLabel: 'Variante', icon: 'qr_code_2', route: '/variante', fkParam: 'colorId' }
  ];
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('color');
  ...
  readonly descriptor = COLOR_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'codigoHex', label: 'Color hex', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Color) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('color', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('color');
  }
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('color-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[ColorList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[ColorList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Color): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[ColorList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Color) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[ColorList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'color_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'color_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Color[], filename: string): void {
  ...
  openDocuments(item: Color): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Color'
      }
    });
  }

  verFicha(item: Color): void {
    this.router.navigate(['/color', item.id]);
  }
  ...
  getStatusLabel(item: Color): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[ColorList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Color): void {
    this.router.navigate([relation.route], {
