...
@Component({
  ...
  templateUrl: './color-ficha.component.html',
  styleUrl: './color-ficha.component.scss'
})
export class ColorFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Color | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('color', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('color');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.nombre || ('Color #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/color']); }
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Color' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Color/${this.id}`).subscribe({
