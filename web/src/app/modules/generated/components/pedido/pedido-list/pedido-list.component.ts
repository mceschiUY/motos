...
import { PEDIDO_DESCRIPTOR } from '../../../models/pedido.descriptor';
...
export class PedidoListComponent implements OnInit, OnDestroy {
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'PedidoLinea', entityLabel: 'Líneas', icon: 'list', route: '/pedidolinea', fkParam: 'pedidoId' }
  ];
  ...
  readonly kanbanListIds = ['kcol-borrador', 'kcol-confirmado', 'kcol-preparado', 'kcol-despachado', 'kcol-entregado', 'kcol-anulado'];
  // destino → estados de origen permitidos (misma matriz que ciclos-vida.json en el backend)
  ...
  itemsDeEstado(estado: string): Pedido[] {
    return this.filteredItems().filter(i => this.normEstado((i as any).estado) === estado);
  }

  itemsSinEstado(): Pedido[] {
    const conocidos = ['borrador', 'confirmado', 'preparado', 'despachado', 'entregado', 'anulado'];
    return this.filteredItems().filter(i => !conocidos.includes(this.normEstado((i as any).estado)));
  }

  puedeEntrarEn(destino: string): (drag: CdkDrag<Pedido>) => boolean {
    return (drag: CdkDrag<Pedido>) => {
      const desde = this.normEstado((drag.data as any)?.estado);
      return (this.kanbanPermitidos[destino] || []).includes(desde);
    };
  }

  onKanbanDrop(event: CdkDragDrop<Pedido[]>, destino: string): void {
  ...
  private kanbanAccion(destino: string): ((id: number | string) => Observable<boolean>) | null {
    switch (destino) {
      case 'confirmado': return (id: number | string) => this.service.pasarAConfirmado(id);
      case 'preparado': return (id: number | string) => this.service.pasarAPreparado(id);
  ...
  claseDeEstado(item: any): string {
    const estados = ['borrador', 'confirmado', 'preparado', 'despachado', 'entregado', 'anulado'];
  ...
  readonly selectedItem = signal<Pedido | null>(null);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('pedido');
  ...
  readonly descriptor = PEDIDO_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'clienteDisplay', label: 'Cliente', visible: true, sortable: true },
    { key: 'estado', label: 'Estado', visible: true, sortable: true },
    { key: 'totalUsd', label: 'Total US$', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Pedido) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params: any) => {
      if (params['clienteId']) {
        ...
        this.breadcrumbItems = [
          ...
          { label: `Pedido de Cliente #${this.filterByClienteId}`, icon: 'list_alt' }
        ];
      }
      if (params['vendedorId']) {
        this.filterByVendedorId = +params['vendedorId'];
        this.filterContext = `Vendedor #${this.filterByVendedorId}`;
        // Actualizar breadcrumb para mostrar contexto
        this.breadcrumbItems = [
          { label: 'Inicio', route: '/', icon: 'home' },
          { label: 'Vendedor', route: '/vendedor', icon: 'badge' },
          { label: `Pedidos de Vendedor #${this.filterByVendedorId}`, icon: 'receipt_long' }
        ];
      }
      this.loadData();
    });
    ...
    this.puenteVoz.registrarAbridor('pedido', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('pedido');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('pedido');
    ...
    const saved = localStorage.getItem('pedido-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('pedido-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[PedidoList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
    ...
    this.service.getAll().subscribe({
      next: (data: Pedido[]) => {
        ...
        if (this.filterByVendedorId) {
          filteredData = filteredData.filter((item: any) => item.vendedorId === this.filterByVendedorId);
        }
        this.items.set(filteredData);
        this.isLoading.set(false);
      },
      error: (err: unknown) => {
        console.error('[PedidoList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  // CRUD
  // ═══════════════════════════════════════════════════════════════════════════
  openForm(item?: Pedido): void {
    // Al crear desde una lista filtrada por un padre, el FK del padre ya viene dado por contexto.
    const contextoFk = this.filterByClienteId ? { campo: 'clienteId', valor: this.filterByClienteId } : this.filterByVendedorId ? { campo: 'vendedorId', valor: this.filterByVendedorId } : null;
  ...
  async deleteItem(item: Pedido): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[PedidoList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Pedido) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[PedidoList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'pedido_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'pedido_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Pedido[], filename: string): void {
  ...
  openDocuments(item: Pedido): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Pedido'
      }
    });
  }

  verFicha(item: Pedido): void {
    this.router.navigate(['/pedido', item.id]);
  }
  ...
  async pasarAConfirmado(item: Pedido): Promise<void> {
    if (await (window as any).confirmar('¿Confirmar el pedido ' + this.getItemTitle(item) + '?')) {
      this.service.pasarAConfirmado(item.id).subscribe({
        next: () => {
          this.loadData();
          this.showMessage('Confirmar: ' + this.getItemTitle(item), 'success');
        },
        error: (err: any) => {
          console.error('[PedidoList] Error en pasarAConfirmado:', err);
          // El backend explica POR QUÉ (sin líneas, sin agencia, transición inválida).
          this.showMessage(err?.error?.detail || 'No se pudo completar la acción', 'error');
        }
      });
    }
  }

  async pasarAPreparado(item: Pedido): Promise<void> {
    if (await (window as any).confirmar('¿Marcar como preparado el pedido ' + this.getItemTitle(item) + '?')) {
      this.service.pasarAPreparado(item.id).subscribe({
        next: () => {
          this.loadData();
          this.showMessage('Marcar como preparado: ' + this.getItemTitle(item), 'success');
        },
        error: (err: any) => {
          console.error('[PedidoList] Error en pasarAPreparado:', err);
          // El backend explica POR QUÉ (sin líneas, sin agencia, transición inválida).
          this.showMessage(err?.error?.detail || 'No se pudo completar la acción', 'error');
        }
      });
  ...
  async pasarADespachado(item: Pedido): Promise<void> {
    if (await (window as any).confirmar('¿Despachar el pedido ' + this.getItemTitle(item) + '?')) {
      this.service.pasarADespachado(item.id).subscribe({
        next: () => {
          this.loadData();
          this.showMessage('Despachar: ' + this.getItemTitle(item), 'success');
        },
        error: (err: any) => {
          console.error('[PedidoList] Error en pasarADespachado:', err);
          // El backend explica POR QUÉ (sin líneas, sin agencia, transición inválida).
          this.showMessage(err?.error?.detail || 'No se pudo completar la acción', 'error');
        }
      });
  ...
  async pasarAEntregado(item: Pedido): Promise<void> {
    if (await (window as any).confirmar('¿Confirmar la entrega el pedido ' + this.getItemTitle(item) + '?')) {
      this.service.pasarAEntregado(item.id).subscribe({
        next: () => {
          this.loadData();
          this.showMessage('Confirmar la entrega: ' + this.getItemTitle(item), 'success');
        },
        error: (err: any) => {
          console.error('[PedidoList] Error en pasarAEntregado:', err);
          // El backend explica POR QUÉ (sin líneas, sin agencia, transición inválida).
          this.showMessage(err?.error?.detail || 'No se pudo completar la acción', 'error');
        }
      });
  ...
  async pasarAAnulado(item: Pedido): Promise<void> {
    if (await (window as any).confirmar('¿Anular el pedido ' + this.getItemTitle(item) + '?')) {
      this.service.pasarAAnulado(item.id).subscribe({
        next: () => {
          this.loadData();
          this.showMessage('Anular: ' + this.getItemTitle(item), 'success');
        },
        error: (err: any) => {
          console.error('[PedidoList] Error en pasarAAnulado:', err);
          // El backend explica POR QUÉ (sin líneas, sin agencia, transición inválida).
          this.showMessage(err?.error?.detail || 'No se pudo completar la acción', 'error');
        }
      });
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: Pedido): string {
    return (item as any).numero || `#${item.id}`;
  }

  getItemSubtitle(item: Pedido): string {
    return (item as any).descripcion || (item as any).email || '';
  }

  getStatusClass(item: Pedido): string {
  ...
  getStatusLabel(item: Pedido): string {
    const status = (item as any).estado;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[PedidoList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Pedido): void {
    this.router.navigate([relation.route], {
