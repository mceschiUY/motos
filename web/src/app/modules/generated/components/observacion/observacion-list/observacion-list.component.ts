...
import { OBSERVACION_DESCRIPTOR } from '../../../models/observacion.descriptor';
...
@Component({
  ...
  styleUrl: './observacion-list.component.scss'
})
export class ObservacionListComponent implements OnInit, OnDestroy {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // CALENDARIO — agenda mensual por fechaHora
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  itemsDeDia(d: Date): Observacion[] {
    return this.filteredItems().filter(i => {
      const f = (i as any).fechaHora;
      if (!f) { return false; }
      const fd = new Date(f);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('observacion');
  ...
  readonly descriptor = OBSERVACION_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'texto', label: 'Texto', visible: true, sortable: true },
    { key: 'fechaHora', label: 'Fecha Hora', visible: true, sortable: true },
    { key: 'usuario', label: 'Usuario', visible: true, sortable: true },
    { key: 'envioId', label: 'Envio', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Observacion) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  readonly gruposPorFecha = computed<TimelineGroup[]>(() => {
    ...
    items.forEach((item: Observacion) => {
      const fecha = (item as any).fechaHora ? new Date((item as any).fechaHora) : new Date();
      ...
      .map(([periodo, items]) => {
        ...
        return {
          ...
          items: items.sort((a, b) => {
            const dateA = (a as any).fechaHora ? new Date((a as any).fechaHora).getTime() : 0;
            const dateB = (b as any).fechaHora ? new Date((b as any).fechaHora).getTime() : 0;
            return dateB - dateA;
          }),
          count: items.length
        };
  ...
  dataSource = new MatTableDataSource<Observacion>([]);
  selection = new SelectionModel<Observacion>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Observación', icon: 'list_alt' }
  ];

  // Parámetros de navegación entre entidades
  filterByEnvioId: number | null = null;
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params: any) => {
      if (params['envioId']) {
        this.filterByEnvioId = +params['envioId'];
        this.filterContext = `Envio #${this.filterByEnvioId}`;
        // Actualizar breadcrumb para mostrar contexto
        this.breadcrumbItems = [
          { label: 'Inicio', route: '/', icon: 'home' },
          { label: 'Envio', route: '/envio', icon: 'list_alt' },
          { label: `Observación de Envio #${this.filterByEnvioId}`, icon: 'list_alt' }
        ];
      }
      this.loadData();
    });
    ...
    this.puenteVoz.registrarAbridor('observacion', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('observacion');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('observacion');
    const validas: string[] = ['table', 'cards', 'master-detail', 'timeline', 'calendario', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('observacion-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('observacion-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[ObservacionList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      next: (data: Observacion[]) => {
        ...
        if (this.filterByEnvioId) {
          filteredData = filteredData.filter((item: any) => item.envioId === this.filterByEnvioId);
        }
        this.items.set(filteredData);
        this.isLoading.set(false);
      },
      error: (err: unknown) => {
        console.error('[ObservacionList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  toggleRow(row: Observacion): void {
  ...
  selectItem(item: Observacion): void {
  ...
  // CRUD
  // ═══════════════════════════════════════════════════════════════════════════
  openForm(item?: Observacion): void {
    // Al crear desde una lista filtrada por un padre, el FK del padre ya viene dado por contexto.
    const contextoFk = this.filterByEnvioId ? { campo: 'envioId', valor: this.filterByEnvioId } : null;
    const dialogRef = this.dialog.open(ObservacionFormComponent, {
      width: '600px',
      maxWidth: '95vw',
  ...
  async deleteItem(item: Observacion): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[ObservacionList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Observacion) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[ObservacionList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'observacion_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'observacion_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Observacion[], filename: string): void {
  ...
  openDocuments(item: Observacion): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Observacion'
      }
    });
  }

  verFicha(item: Observacion): void {
    this.router.navigate(['/observacion', item.id]);
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: Observacion): string {
    return (item as any).texto || (item as any).nombre || (item as any).titulo || `#${item.id}`;
  }

  getItemSubtitle(item: Observacion): string {
    return (item as any).descripcion || (item as any).email || '';
  }

  getStatusClass(item: Observacion): string {
  ...
  getStatusLabel(item: Observacion): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[ObservacionList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Observacion): void {
    this.router.navigate([relation.route], {
