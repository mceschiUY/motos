...
@Component({
  selector: 'app-observacion-ficha',
  standalone: true,
  ...
  templateUrl: './observacion-ficha.component.html',
  styleUrl: './observacion-ficha.component.scss'
})
export class ObservacionFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Observacion | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('observacion', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('observacion');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.texto || ('Observación #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/observacion']); }
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Observacion' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Observacion/${this.id}`).subscribe({
