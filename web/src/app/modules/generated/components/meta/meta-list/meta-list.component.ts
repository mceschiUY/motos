...
import { META_DESCRIPTOR } from '../../../models/meta.descriptor';
...
export class MetaListComponent implements OnInit, OnDestroy {
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('meta');
  ...
  readonly descriptor = META_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'periodo', label: 'Período', visible: true, sortable: true },
    { key: 'vendedorDisplay', label: 'Vendedor', visible: true, sortable: true },
    { key: 'objetivoUsd', label: 'Objetivo US$', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Meta) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  dataSource = new MatTableDataSource<Meta>([]);
  selection = new SelectionModel<Meta>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Meta', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('meta', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('meta');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('meta');
    const validas: string[] = ['table', 'cards', 'master-detail', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('meta-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('meta-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[MetaList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[MetaList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Meta): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[MetaList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Meta) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[MetaList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'meta_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'meta_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Meta[], filename: string): void {
  ...
  verFicha(item: Meta): void {
    this.router.navigate(['/meta', item.id]);
  }
  ...
  getStatusLabel(item: Meta): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[MetaList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Meta): void {
    this.router.navigate([relation.route], {
