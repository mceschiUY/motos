...
import { CATEGORIA_DESCRIPTOR } from '../../../models/categoria.descriptor';
...
export class CategoriaListComponent implements OnInit, OnDestroy {
  ...
  readonly selectedItem = signal<Categoria | null>(null);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('categoria');
  ...
  readonly descriptor = CATEGORIA_DESCRIPTOR;
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Categoria) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('categoria', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('categoria');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('categoria');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('categoria-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('categoria-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[CategoriaList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[CategoriaList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  selectItem(item: Categoria): void {
  ...
  async deleteItem(item: Categoria): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[CategoriaList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Categoria) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[CategoriaList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'categoria_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'categoria_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Categoria[], filename: string): void {
  ...
  openDocuments(item: Categoria): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Categoria'
      }
    });
  }

  verFicha(item: Categoria): void {
  ...
  getStatusLabel(item: Categoria): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[CategoriaList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Categoria): void {
    this.router.navigate([relation.route], {
