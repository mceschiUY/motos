...
import { VENDEDOR_DESCRIPTOR } from '../../../models/vendedor.descriptor';
...
export class VendedorListComponent implements OnInit, OnDestroy {
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'Cliente', entityLabel: 'Clientes', icon: 'person', route: '/cliente', fkParam: 'vendedorId' },
    { entityName: 'Actividad', entityLabel: 'Actividades', icon: 'event_note', route: '/actividad', fkParam: 'vendedorId' },
    { entityName: 'Meta', entityLabel: 'Metas', icon: 'flag', route: '/meta', fkParam: 'vendedorId' }
  ];
  ...
  readonly selectedItem = signal<Vendedor | null>(null);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('vendedor');
  ...
  readonly descriptor = VENDEDOR_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'zona', label: 'Zona', visible: true, sortable: true },
    { key: 'comisionPorcentaje', label: 'Comisión %', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Vendedor) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  readonly gruposPorFecha = computed<TimelineGroup[]>(() => {
    ...
    const grupos = new Map<string, Vendedor[]>();

    items.forEach((item: Vendedor) => {
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('vendedor', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('vendedor');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('vendedor');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('vendedor-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('vendedor-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[VendedorList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[VendedorList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Vendedor): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[VendedorList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Vendedor) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[VendedorList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'vendedor_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'vendedor_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Vendedor[], filename: string): void {
  ...
  openDocuments(item: Vendedor): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Vendedor'
      }
    });
  }

  verFicha(item: Vendedor): void {
    this.router.navigate(['/vendedor', item.id]);
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: Vendedor): string {
    return (item as any).nombre || (item as any).nombre || (item as any).titulo || `#${item.id}`;
  }

  getItemSubtitle(item: Vendedor): string {
    return (item as any).zona || (item as any).email || '';
  }

  getStatusClass(item: Vendedor): string {
  ...
  getStatusLabel(item: Vendedor): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[VendedorList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Vendedor): void {
    this.router.navigate([relation.route], {
