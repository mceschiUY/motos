...
import { VARIANTE_DESCRIPTOR } from '../../../models/variante.descriptor';
...
@Component({
  ...
  styleUrl: './variante-list.component.scss'
})
export class VarianteListComponent implements OnInit, OnDestroy {
  private readonly service = inject(VarianteService);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('variante');
  ...
  readonly descriptor = VARIANTE_DESCRIPTOR;
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Variante) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  dataSource = new MatTableDataSource<Variante>([]);
  selection = new SelectionModel<Variante>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Variante', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('variante', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('variante');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('variante');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('variante-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('variante-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[VarianteList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[VarianteList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Variante): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[VarianteList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Variante) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[VarianteList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'variante_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'variante_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Variante[], filename: string): void {
  ...
  openDocuments(item: Variante): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Variante'
      }
    });
  }

  verFicha(item: Variante): void {
    this.router.navigate(['/variante', item.id]);
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: Variante): string {
    return (item as any).sku || (item as any).nombre || `#${item.id}`;
  }

  getItemSubtitle(item: Variante): string {
    return (item as any).descripcion || (item as any).email || '';
  }

  getStatusClass(item: Variante): string {
  ...
  getStatusLabel(item: Variante): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[VarianteList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Variante): void {
    this.router.navigate([relation.route], {
