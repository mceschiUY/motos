...
@Component({
  ...
  templateUrl: './variante-ficha.component.html',
  styleUrl: './variante-ficha.component.scss'
})
export class VarianteFichaComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('variante', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('variante');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.sku || ('Variante #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/variante']); }
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Variante' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Variante/${this.id}`).subscribe({
