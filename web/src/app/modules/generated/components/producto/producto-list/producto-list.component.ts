...
import { PRODUCTO_DESCRIPTOR } from '../../../models/producto.descriptor';
...
export class ProductoListComponent implements OnInit, OnDestroy {
  ...
  readonly selectedItem = signal<Producto | null>(null);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('producto');
  ...
  readonly descriptor = PRODUCTO_DESCRIPTOR;
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Producto) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('producto', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('producto');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('producto');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('producto-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('producto-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[ProductoList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[ProductoList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Producto): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[ProductoList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Producto) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[ProductoList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'producto_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'producto_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Producto[], filename: string): void {
  ...
  openDocuments(item: Producto): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Producto'
      }
    });
  }

  verFicha(item: Producto): void {
  ...
  getStatusLabel(item: Producto): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[ProductoList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Producto): void {
    this.router.navigate([relation.route], {
