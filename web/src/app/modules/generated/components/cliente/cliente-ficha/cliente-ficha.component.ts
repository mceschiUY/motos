...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, MatIconModule, MatButtonModule, MatTooltipModule, MatDialogModule, MatSnackBarModule, FotoGaleriaComponent, MembreteImpresionComponent, MiniMapaComponent],
...
export class ClienteFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Cliente | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('cliente', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('cliente');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.nombre || ('Cliente #' + i.id)) : '';
  }
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Cliente' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Cliente/${this.id}`).subscribe({
