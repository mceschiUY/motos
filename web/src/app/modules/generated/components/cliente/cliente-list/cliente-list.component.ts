...
export class ClienteListComponent implements OnInit, OnDestroy {
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'Envio', entityLabel: 'Envio', icon: 'list_alt', route: '/envio', fkParam: 'clienteId' }
  ];
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('cliente');
  ...
  readonly descriptor = CLIENTE_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'telefono', label: 'Teléfono', visible: true, sortable: true },
    { key: 'direccionEntrega', label: 'Dirección Entrega', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Cliente) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('cliente', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('cliente');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('cliente');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('cliente-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('cliente-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[ClienteList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    origen.subscribe({
      ...
      error: (err: unknown) => {
        console.error('[ClienteList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Cliente): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[ClienteList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Cliente) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[ClienteList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'cliente_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'cliente_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Cliente[], filename: string): void {
  ...
  openDocuments(item: Cliente): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Cliente'
      }
    });
  }

  verFicha(item: Cliente): void {
  ...
  getStatusLabel(item: Cliente): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[ClienteList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Cliente): void {
    this.router.navigate([relation.route], {
