...
@Component({
  ...
  templateUrl: './marca-ficha.component.html',
  styleUrl: './marca-ficha.component.scss'
})
export class MarcaFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Marca | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('marca', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('marca');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.nombre || ('Marca #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/marca']); }

  editar(): void {
    const ref = this.dialog.open(MarcaFormComponent, {
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Marca' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Marca/${this.id}`).subscribe({
