...
import { MARCA_DESCRIPTOR } from '../../../models/marca.descriptor';
...
export class MarcaListComponent implements OnInit, OnDestroy {
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'Producto', entityLabel: 'Producto', icon: 'inventory_2', route: '/producto', fkParam: 'marcaId' }
  ];
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('marca');
  ...
  readonly descriptor = MARCA_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'pais', label: 'País', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Marca) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  selection = new SelectionModel<Marca>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Marca', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('marca', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('marca');
  }
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('marca-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[MarcaList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[MarcaList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Marca): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[MarcaList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Marca) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[MarcaList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'marca_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'marca_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Marca[], filename: string): void {
  ...
  openDocuments(item: Marca): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Marca'
      }
    });
  }

  verFicha(item: Marca): void {
    this.router.navigate(['/marca', item.id]);
  }
  ...
  getStatusLabel(item: Marca): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[MarcaList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Marca): void {
    this.router.navigate([relation.route], {
