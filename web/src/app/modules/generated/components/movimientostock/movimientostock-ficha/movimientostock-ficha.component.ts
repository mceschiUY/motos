...
@Component({
  selector: 'app-movimientostock-ficha',
  standalone: true,
  ...
  templateUrl: './movimientostock-ficha.component.html',
  styleUrl: './movimientostock-ficha.component.scss'
})
export class MovimientoStockFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<MovimientoStock | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('movimientostock', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('movimientostock');
  }
  ...
  titulo(): string {
    ...
    return i ? ((i.documentoOrigen || i.tipo || 'Movimiento') + ' #' + i.id) : '';
  }

  volver(): void { this.router.navigate(['/movimientostock']); }
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'MovimientoStock' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/MovimientoStock/${this.id}`).subscribe({
