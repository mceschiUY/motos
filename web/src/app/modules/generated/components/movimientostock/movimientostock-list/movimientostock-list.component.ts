...
import { MOVIMIENTOSTOCK_DESCRIPTOR } from '../../../models/movimientostock.descriptor';
import { MovimientoStockService } from '../../../services/movimientostock.service';
import { MovimientoStockFormComponent } from '../movimientostock-form/movimientostock-form.component';
...
@Component({
  selector: 'app-movimientostock-list',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './movimientostock-list.component.html',
  styleUrl: './movimientostock-list.component.scss'
})
export class MovimientoStockListComponent implements OnInit, OnDestroy {
  private readonly service = inject(MovimientoStockService);
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('movimientostock');
  ...
  readonly descriptor = MOVIMIENTOSTOCK_DESCRIPTOR;
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: MovimientoStock) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  selection = new SelectionModel<MovimientoStock>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'MovimientoStock', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('movimientostock', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('movimientostock');
  }
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('movimientostock-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[MovimientoStockList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[MovimientoStockList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  toggleRow(row: MovimientoStock): void {
  ...
  selectItem(item: MovimientoStock): void {
  ...
  async deleteItem(item: MovimientoStock): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[MovimientoStockList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: MovimientoStock) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[MovimientoStockList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'movimientostock_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'movimientostock_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: MovimientoStock[], filename: string): void {
  ...
  openDocuments(item: MovimientoStock): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'MovimientoStock'
      }
    });
  }

  verFicha(item: MovimientoStock): void {
    this.router.navigate(['/movimientostock', item.id]);
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  getItemTitle(item: MovimientoStock): string {
    return (item as any).documentoOrigen || (item as any).tipo || `#${item.id}`;
  }

  getItemSubtitle(item: MovimientoStock): string {
    return (item as any).descripcion || (item as any).email || '';
  }

  getStatusClass(item: MovimientoStock): string {
  ...
  getStatusLabel(item: MovimientoStock): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[MovimientoStockList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: MovimientoStock): void {
    this.router.navigate([relation.route], {
