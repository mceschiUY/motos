...
@Component({
  selector: 'app-pedidolinea-ficha',
  standalone: true,
  ...
  templateUrl: './pedidolinea-ficha.component.html',
  styleUrl: './pedidolinea-ficha.component.scss'
})
export class PedidoLineaFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<PedidoLinea | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('pedidolinea', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('pedidolinea');
  }
  ...
  titulo(): string {
    ...
    if (!i) { return ''; }
    return (i.varianteDisplay || ('Línea #' + i.id)) + (i.pedidoDisplay ? ' · ' + i.pedidoDisplay : '');
  }

  volver(): void { this.router.navigate(['/pedidolinea']); }

  editar(): void {
    const ref = this.dialog.open(PedidoLineaFormComponent, {
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'PedidoLinea' } });
  }
  ...
  irAPedido(): void {
    const i = this.item();
    if (i?.pedidoId) { this.router.navigate(['/pedido', i.pedidoId]); }
  }

  // ═══ Historia: la auditoría que el sistema YA registra, por fin visible ═══
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/PedidoLinea/${this.id}`).subscribe({
