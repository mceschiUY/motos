...
import { DEPOSITO_DESCRIPTOR } from '../../../models/deposito.descriptor';
...
export class DepositoListComponent implements OnInit, OnDestroy {
  private readonly service = inject(DepositoService);
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'MovimientoStock', entityLabel: 'Movimiento de stock', icon: 'swap_vert', route: '/movimientostock', fkParam: 'depositoId' }
  ];
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('deposito');
  ...
  readonly descriptor = DEPOSITO_DESCRIPTOR;
  ...
  readonly columnConfigs = signal<ColumnConfig[]>([
    ...
    { key: 'direccion', label: 'Dirección', visible: true, sortable: true },
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Deposito) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  readonly gruposPorFecha = computed<TimelineGroup[]>(() => {
    ...
    const grupos = new Map<string, Deposito[]>();

    items.forEach((item: Deposito) => {
  ...
  selection = new SelectionModel<Deposito>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Deposito', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('deposito', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('deposito');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('deposito');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('deposito-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('deposito-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[DepositoList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[DepositoList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Deposito): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[DepositoList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Deposito) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[DepositoList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'deposito_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'deposito_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Deposito[], filename: string): void {
  ...
  openDocuments(item: Deposito): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Deposito'
      }
    });
  }

  verFicha(item: Deposito): void {
    this.router.navigate(['/deposito', item.id]);
  }
  ...
  getStatusLabel(item: Deposito): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[DepositoList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Deposito): void {
    this.router.navigate([relation.route], {
