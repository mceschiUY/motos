...
import { AGENCIA_DESCRIPTOR } from '../../../models/agencia.descriptor';
...
@Component({
  ...
  styleUrl: './agencia-list.component.scss'
})
export class AgenciaListComponent implements OnInit, OnDestroy {
  private readonly service = inject(AgenciaService);
  ...
  readonly hasManyRelations: HasManyRelation[] = [
    { entityName: 'Envio', entityLabel: 'Envio', icon: 'list_alt', route: '/envio', fkParam: 'agenciaId' }
  ];
  ...
  readonly artesanales: VistaArtesanal[] = vistasArtesanalesDe('agencia');
  ...
  readonly descriptor = AGENCIA_DESCRIPTOR;
  ...
  readonly filteredItems = computed(() => {
    ...
    return items.filter((item: Agencia) => JSON.stringify(item).toLowerCase().includes(search));
  });
  ...
  readonly gruposPorFecha = computed<TimelineGroup[]>(() => {
    ...
    const grupos = new Map<string, Agencia[]>();

    items.forEach((item: Agencia) => {
  ...
  selection = new SelectionModel<Agencia>(true, []);
  readonly skeletonRows = Array(5).fill(0);

  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Inicio', route: '/', icon: 'home' },
    { label: 'Agencia', icon: 'list_alt' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarAbridor('agencia', () => this.openForm());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarAbridor('agencia');
  }
  ...
  private loadViewMode(): ViewMode {
    ...
    const artesanales = vistasArtesanalesDe('agencia');
    const validas: string[] = ['table', 'cards', 'master-detail', 'with-relations', ...artesanales.map(v => 'artesanal:' + v.key)];
    const saved = localStorage.getItem('agencia-view-mode-v2');
  ...
  setViewMode(mode: string): void {
    ...
    localStorage.setItem('agencia-view-mode-v2', mode);
  }
  ...
  private cargarArtesanal(key: string): void {
    ...
    vista.loadComponent()
      ...
      .catch((err: unknown) => {
        console.warn('[AgenciaList] Vista artesanal «' + key + '» rota — caigo a tabla (paracaídas):', err);
        this.artesanalCargando.set(false);
        this.setViewMode('table');
      });
  }
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: (err: unknown) => {
        console.error('[AgenciaList] Error cargando datos:', err);
        this.isLoading.set(false);
  ...
  async deleteItem(item: Agencia): Promise<void> {
    if (await (window as any).confirmar('¿Eliminar este registro?\\n\\nEsta acción no se puede deshacer.')) {
      this.service.delete(item.id).subscribe({
        ...
        error: (err: unknown) => {
          console.error('[AgenciaList] Error eliminando:', err);
  ...
  async deleteSelected(): Promise<void> {
    ...
    if (await (window as any).confirmar(`¿Eliminar ${count} registro(s)?\\n\\nEsta acción no se puede deshacer.`)) {
      const deleteObservables = this.selection.selected.map((item: Agencia) => this.service.delete(item.id));
      import('rxjs').then(({ forkJoin }) => {
        forkJoin(deleteObservables).subscribe({
          ...
          error: (err: unknown) => {
            console.error('[AgenciaList] Error eliminando múltiples:', err);
            this.loadData();
  ...
  exportSelected(): void {
    ...
    this.exportData(data, 'agencia_seleccionados');
  ...
  exportAll(): void {
    ...
    this.exportData(data, 'agencia_todos');
    this.showMessage('Archivo CSV exportado', 'success');
  }

  private exportData(data: Agencia[], filename: string): void {
  ...
  openDocuments(item: Agencia): void {
    this.router.navigate(['/documento'], {
      queryParams: {
        relacionId: item.id,
        relacionNombre: 'Agencia'
      }
    });
  }

  verFicha(item: Agencia): void {
    this.router.navigate(['/agencia', item.id]);
  }
  ...
  getStatusLabel(item: Agencia): string {
    const status = (item as any).activo;
  ...
  private loadRelatedItems(entityNameCamel: string, parentId: string | number): void {
    ...
    this.http.get<any[]>(url).subscribe({
      ...
      error: (err: unknown) => {
        console.error(`[AgenciaList] Error cargando ${relation.entityName}:`, err);
   ...
   */
  navigateToChild(relation: HasManyRelation, item: Agencia): void {
    this.router.navigate([relation.route], {
