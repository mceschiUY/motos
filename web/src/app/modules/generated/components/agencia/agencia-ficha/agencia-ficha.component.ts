...
@Component({
  ...
  templateUrl: './agencia-ficha.component.html',
  styleUrl: './agencia-ficha.component.scss'
})
export class AgenciaFichaComponent implements OnInit, OnDestroy {
  ...
  readonly item = signal<Agencia | null>(null);
  ...
  readonly relaciones: RelacionFicha[] = [
    { nombre: 'envio', label: 'Envios', icon: 'list_alt', ruta: '/envio', endpoint: '/Envio/by-agencia/' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.puenteVoz.registrarEditor('agencia', () => this.editar());
  }

  ngOnDestroy(): void {
    this.puenteVoz.quitarEditor('agencia');
  }
  ...
  titulo(): string {
    ...
    return i ? (i.nombre || ('Agencia #' + i.id)) : '';
  }

  volver(): void { this.router.navigate(['/agencia']); }

  editar(): void {
    const ref = this.dialog.open(AgenciaFormComponent, {
  ...
  documentos(): void {
    this.router.navigate(['/documento'], { queryParams: { relacionId: this.id, relacionNombre: 'Agencia' } });
  }
  ...
  cargarHistoria(): void {
    this.http.get<any[]>(`${environment.apiUrl}/Auditoria/entidad/Agencia/${this.id}`).subscribe({
