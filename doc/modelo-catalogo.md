...
- [x] 4. Seeds base (tallas, colores, categorías, marcas, depósitos) + datos de demo (10 productos, 73 variantes, Kardex, clientes y envíos) — viven en `Scripts/Seed_Dominio_Motos.sql`, que corren `DbBootstrap` (Development) e `init-db.sh` (Docker). Ya no hay seeds dentro de los `PC_*.sql`. Limpieza a pedido: `Scripts/Limpiar_Datos_Dominio.sql`.
- [x] 5. Enum-pills: los enums (Genero/TipoCasco/Homologacion, y Envio.estado / MovimientoStock.tipo) se declaran en el descriptor con `tipo:'enum'` + `valores`. La tabla genérica (`core/components/entity-table`) los renderiza como `<span class="enum-pill" data-valor="…">` y el mapeo valor→acento vive en `styles.scss` §16 (paridad con trenes, 2026-09-07).
